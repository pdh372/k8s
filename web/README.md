...
- **Interactive diagrams** — eight clickable diagrams (cluster architecture,
  networking, Ingress/Service traffic, storage, RBAC/security, certificates &
  kubeconfig, CI/CD, and observability); click any component to see what it does,
...
- `src/data/architecture.ts`, `networking.ts`, `traffic.ts`, `storage.ts`,
  ...
  by exporting another `Diagram` and appending it to the `DIAGRAMS` array.
